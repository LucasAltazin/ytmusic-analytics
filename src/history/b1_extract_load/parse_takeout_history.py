...
OUTPUT_CSV = PROJECT_ROOT / "data" / "takeout_history_mvp.csv"
...
def extract_title_artist_album(entry: dict):
    ...
    return track_title.strip(), artist.strip(), album.strip()
...
def main():
    ...
    for json_file in iter_history_files():
        ...
        for entry in data:
            ...
            track_title, artist, album = extract_title_artist_album(entry)
            header = entry.get("header", "")

            # ignore YouTube (video) history, keep only YouTube Music
